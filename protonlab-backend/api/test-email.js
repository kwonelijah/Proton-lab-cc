...
export default async function handler(req, res) {
  ...
  const order = {
    ...
    ref: 'Edinburgh Bike Fitting Club #04827',
    number: '04827',
    amount: '170.00',
    currency: 'GBP',
    ...
    lineItems: [
      { name: 'SS Race Jersey', handle: 'ss-race-jersey', size: 'M', qty: 1 },
      { name: 'Race Bib Shorts', handle: 'race-bib-shorts', size: 'L', qty: 1 },
    ],
    shipping: {
      name: 'Elijah Kwon',
  ...
  res.status(200).json({
    resendKeyPresent: Boolean(process.env.proton_resend_key || process.env.Resend_Backend_Key),
